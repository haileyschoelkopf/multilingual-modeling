...
if args.zero_shot:
    print("Cross Lingual")
    en_dataset = load_dataset("wikiann", "english", cache_dir=args.cache_dir)
    dataset = load_dataset("wikiann", args.lang, cache_dir=args.cache_dir)
...
if args.zero_shot:
    def tokenize_function(example):
        sent = ' '.join(example["tokens"])
        inputs = en_tokenizer(f'Given the following sentence: {sent} what is the crosponding \
        named entity spans if any?', max_length=128, padding="max_length", truncation=True)
        inputs["labels"] = ' '.join(example["spans"])
        return inputs
...
def load_model(args, inference=False):
    ...
    if not inference:
        ...
        if args.finetune_strategies == "whole":
            ...
            model.add_adapter("wikiann-task-adapter")
            model.train_adapter("wikiann-task-adapter")
        else:
        ...
        if args.finetune_strategies == "lang_adapters":
            ...
            if args.madx_lang_adapter:
                ...
                adapter_name = model.load_adapter(args.madx_lang_adapter)
                model.set_active_adapters(adapter_name)
                adapter_name = model.load_adapter(f"{args.pretrained_adapters_dir}/wikiann-task-adapter")
                model.set_active_adapters(adapter_name)
            else:
                adapter_name = model.load_adapter(f"{args.pretrained_adapters_dir}/wikiann-task-adapter") #TODO: change the argument to this
                model.set_active_adapters(adapter_name)
        # print(model)
